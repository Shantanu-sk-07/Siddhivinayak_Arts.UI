...
import NotificationBell from "@/view/DashboardPages/SuperAdmin/NotificationDetails";
...
export default function DashboardLayout() {
  ...
  const [festivalAnchorEl, setFestivalAnchorEl] = useState<null | HTMLElement>(null);
  const [currentTime, setCurrentTime] = useState(new Date());
  const { user, logout } = useAuth();
  const { hasPermission, isAdmin, isCustomer } = usePermissions();
  ...
  // SUPER_ADMIN gets all tabs (including former staff tabs)
  const getNavItems = (): NavItemType[] => {
    ...
    if (isAdmin) {
      items.push(
        ...
        { text: "Pickup Management", icon: <ShoppingBag />, path: "/admin/pickups" },  // Former staff page
        { text: "QR Scan Verification", icon: <QrCodeScanner />, path: "/admin/scan" }, // Former staff page
  ...
  const getGreeting = () => {
    ...
    if (hour < 12) return { text: "Good Morning", icon: <WbSunny />, color: "#ff0" };
    if (hour < 17) return { text: "Good Afternoon", icon: <WbSunny />, color: "#ff9800" };
    ...
    <Box sx={{ height: "100%", display: "flex", flexDirection: "column", pt: 2 }}>
      <Box sx={{ px: 2.5, pb: 2.5, display: "flex", alignItems: "center", gap: 1.5 }}>
        ...
        <Box>
          ...
          <Typography variant="caption" sx={{ opacity: 0.85, color: "white", fontSize: "0.7rem" }}>
        ...
        {getNavItems().map((item, index) => (
          ...
          >
            ...
            >
              ...
              {location.pathname === item.path && <KeyboardArrowRight sx={{ fontSize: 18, color: "white" }} />}
            </NavItem>
          </motion.div>
        ))}
      </List>
      ...
      <GreetingCard>
        <CardContent sx={{ p: 2, '&:last-child': { pb: 2 }, textAlign: 'center' }}>
          <Box display="flex" alignItems="center" justifyContent="center" gap={1} mb={0.5}>
            ...
            <Box sx={{ color: greeting.color, display: 'flex', alignItems: 'center' }}>{greeting.icon}</Box>
          </Box>
      ...
      <Box sx={{ p: 2, textAlign: "center" }}>
        <Typography variant="caption" sx={{ color: "white", opacity: 1, fontSize: "1rem" }}>
          🌺 गणपती बाप्पा मोरया 🌺
        </Typography>
      </Box>
    </Box>
        ...
        <Box sx={{ display: "flex" }}>
          <StyledAppBar position="fixed" sx={{ zIndex: theme.zIndex.drawer + 1 }}>
            <Toolbar sx={{ px: { xs: 2, sm: 3 }, minHeight: { xs: 56, sm: 64 } }}>
              <IconButton color="inherit" edge="start" onClick={handleDrawerToggle} sx={{ mr: 2, display: { sm: "none" }, color: "white" }}>
                <MenuIcon />
              </IconButton>
              <Typography variant="h6" sx={{ flex: 1, fontWeight: 600, color: "white", fontSize: { xs: "0.9rem", sm: "1.1rem" } }}>
                Siddhivinayak Arts
              </Typography>
              <Box display="flex" alignItems="center" gap={1.5}>
                <Tooltip title="Festival Calendar">
                  <IconButton onClick={handleFestivalOpen} size="small" sx={{ color: "white" }}>
                    <CalendarToday />
                  </IconButton>
                </Tooltip>
                <NotificationBell />
                <IconButton onClick={handleMenuOpen} size="small">
                  <Avatar sx={{ width: { xs: 32, sm: 36 }, height: { xs: 32, sm: 36 }, bgcolor: alpha(theme.palette.common.white, 0.2), color: "white" }}>
              ...
              <Popover
                ...
                PaperProps={{ sx: { p: 2, mt: 1, borderRadius: 2, minWidth: 200, background: "white" } }}
              >
                <Typography variant="subtitle2" fontWeight={600} gutterBottom>🎉 Ganesh Festival 2026</Typography>
                <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>📅 Start: August 22, 2026</Typography>
                <Typography variant="body2" color="text.secondary">📅 End: September 2, 2026</Typography>
                <Divider sx={{ my: 1 }} />
                <Typography variant="caption" color="primary">🙏 Ganpati Bappa Morya!</Typography>
              </Popover>
              <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleMenuClose} PaperProps={{ sx: { borderRadius: 2, mt: 1, minWidth: 180 } }}>
                ...
                <MenuItem onClick={handleLogout} sx={{ color: theme.palette.error.main }}>
                  <ListItemIcon><Logout fontSize="small" sx={{ color: theme.palette.error.main }} /></ListItemIcon>Logout
                </MenuItem>
              </Menu>
            </Toolbar>
          </StyledAppBar>
          ...
          <Box component="nav" sx={{ width: { sm: drawerWidth }, flexShrink: { sm: 0 } }}>
            <StyledDrawer
              ...
              sx={{ display: { xs: "block", sm: "none" }, "& .MuiDrawer-paper": { width: "85%", maxWidth: 260, top: 0 } }}
            >
              {drawerContent}
            </StyledDrawer>
            <StyledDrawer variant="permanent" sx={{ display: { xs: "none", sm: "block" }, "& .MuiDrawer-paper": { width: drawerWidth, top: 0, position: "fixed" } }} open>
              {drawerContent}
            </StyledDrawer>
          </Box>

          <MainContent sx={{ flexGrow: 1, p: { xs: 2, sm: 3 }, width: { xs: "100%", sm: `calc(100% - ${drawerWidth}px)` }, mt: { xs: 7, sm: 8 }, position: "relative", zIndex: 1, minHeight: "100vh", overflowX: "auto" }}>
