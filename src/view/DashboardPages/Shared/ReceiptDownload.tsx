...
export default function ReceiptDownload({ booking, payments }: ReceiptDownloadProps) {
  const handleDownload = async () => {
    try {
      const response = await fetch(`/api/customer/receipt/${booking.id}`, {
        method: 'POST',
      });
  ...
  return (
    <Box id="receipt-content" sx={{ p: 3 }}>
      ...
      <Box mb={3}>
        ...
        <Typography variant="body2">Booking Date: {new Date(booking.createdAt).toLocaleDateString()}</Typography>
        <Typography variant="body2">Ganpati: {booking.ganpatiName}</Typography>
      </Box>

      <Box mb={3}>
        <Typography variant="subtitle2">Customer Information</Typography>
        <Typography variant="body2">Name: {booking.customerName}</Typography>
      ...
      <Box textAlign="center" mt={3}>
        <Typography variant="caption" color="textSecondary">
          Thank you for choosing Siddhivinayak Arts! Have a blessed festival.
        </Typography>
      </Box>
